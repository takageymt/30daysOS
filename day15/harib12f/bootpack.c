...
void HariMain(void) {
    ...
    int mx, my, i, args[10], cursor_x, cursor_c, task_b_esp;
    ...
    set_segmdesc(gdt + 3, 103, (int) &tss_a, AR_TSS32); // current task is task_a (3)
    set_segmdesc(gdt + 4, 103, (int) &tss_b, AR_TSS32); // other task 

    load_tr(3*8); 
    task_b_esp = memman_alloc_4k(memman, 64*1024) + 64*1024 - 8; // SP to the end of a stack
    tss_b.eip = (int) &task_b_main; // entry point of task_b (4); IP = instruction pointer
    ...
    for (;;) {
        ...
        if (fifo32_status(&fifo) == 0) {
            ...
            if (i == 2) {
                farjmp(0, 4*8);
                timer_settime(timer_ts, 2);
            } else if (256 <= i && i <= 511) { // from a keyboard
                args[0] = i - 256;
                hrb_sprintfi(s, "%02X", args);
                putfonts8_asc_sht(sht_back, 0, 16, COL8_FFFFFF, COL8_008484, s, 2);
                if (i < 256 + 0x54) {
                    if (keytable[i - 256] != 0 && cursor_x < 144) { // normal characters
                        s[0] = keytable[i - 256];
                        s[1] = 0;
