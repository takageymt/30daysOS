        ...
        if (fifo32_status(&fifo) == 0) {
            ...
            if (256 <= i && i <= 511) { // from a keyboard
                ...
                if (s[0] != 0) { // normal, backspace, return
                    if (key_ctrl != 0 && i == 256 + 0x31) { // Ctrl+N: open a new console
                        if (key_win != 0) {
                            keywin_off(key_win);
                        }
                        key_win = open_console(shtctl, memtotal);
                        ...
                        if (key_ctrl != 0 && i == 256 + 0x2e) { // Ctrl+C: force quit
                            task = key_win->task;
                                ...
                                if (0 <= x && x < sht->bxsize && 0 <= y && y < sht->bysize) {
                                    if (sht->buf[y*sht->bxsize + x] != sht->col_inv) {
                                        ...
                                        if (sht->bxsize-21 <= x && x < sht->bxsize-5 && 5 <= y && y < 19 && sht->task != 0) { 
                                            // [x] clicked
                                            if ((sht->flags & 0x10) != 0) { // a window created by app
                                                task = sht->task;
                                                cons_putstr0(task->cons, "\nBreak(mouse) :\n");
                                                io_cli();
                                                task->tss.eax = (int) &(task->tss.esp0);
            ...
            } else if (768 <= i && i <= 1023) { // quit a console
...
struct TASK *open_constask(struct SHEET *sht, unsigned int memtotal) {
    ...
    task->tss.esp = task->cons_stack + 64*1024 - 12; // SP to the end of a stack
    task->tss.eip = (int) &console_task; // entry point of task_b (4); IP = instruction pointer
    task->tss.es = 1*8;
    task->tss.cs = 2*8; // GDT 2 (this OS program)
    task->tss.ss = 1*8;
    task->tss.ds = 1*8;
    task->tss.fs = 1*8;
    task->tss.gs = 1*8;
