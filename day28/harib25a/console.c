...
void console_task(struct SHEET *sheet, unsigned int memtotal) {
    ...
    for (;;) { 
        ...
        if (fifo32_status(&task->fifo) == 0) {
            ...
            io_sti();
            if (i <= 1 && cons.sht != 0) { // for the cursor
                if (i != 0) {
...
void cons_runcmd(char *cmdline, struct CONSOLE *cons, int *fat, unsigned int memtotal) {
    if (hrb_strcmp(cmdline, "free") == 0 && cons->sht != 0) { 
        cmd_free(cons, memtotal);
    } else if (hrb_strcmp(cmdline, "clear") == 0 && cons->sht != 0) {
        cmd_clear(cons);
    } else if (hrb_strcmp(cmdline, "ls") == 0 && cons->sht != 0) {
        cmd_ls(cons);
    } else if (hrb_strncmp(cmdline, "cat ", 4) == 0 && cons->sht != 0) {
        cmd_cat(cons, fat, cmdline);
    } else if (hrb_strcmp(cmdline, "exit") == 0) {
        cmd_exit(cons, fat);
    } else if (hrb_strncmp(cmdline, "start ", 6) == 0) {
        cmd_start(cons, cmdline, memtotal);
    } else if (hrb_strncmp(cmdline, "ncst ", 5) == 0) {
...
void cmd_exit(struct CONSOLE *cons, int *fat) {
    ...
    if (cons->sht != 0) {
        timer_cancel(cons->timer); // cancel the cursor timer
    }
...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline) {
    ...
    if (finfo != 0) {
        ...
        if (finfo->size >= 8 && hrb_strncmp(p+4, "Hari", 4) == 0) {
            ...
            datsz  = *((int *) (p + 0x0010));
            dathrb = *((int *) (p + 0x0014));
            q = (char *) memman_alloc_4k(memman, segsz);
            task->ds_base = (int) q;
            set_segmdesc(task->ldt + 0, finfo->size - 1, (int) p, AR_CODE32_ER + 0x60);
            set_segmdesc(task->ldt + 1,       segsz - 1, (int) q, AR_DATA32_RW + 0x60);
            for (i = 0; i < datsz; i++) {
                q[esp + i] = p[dathrb + i];
            }
            ...
            memman_free_4k(memman, (int) q, segsz);
        } else {
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax) {
    ...
    if (edx == 1) {
        ...
        for (;;) {
            ...
            i = fifo32_get(&task->fifo);
            io_sti();
            if (i <= 1) { // blinking cursor
                timer_init(cons->timer, &task->fifo, 1); // invisible
                timer_settime(cons->timer, 50);
            }
            if (i == 2) { // cursor on
                cons->cur_c = COL8_FFFFFF;
