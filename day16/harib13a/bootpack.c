...
void HariMain(void) {
    ...
    task_init(memman);
    task_b = task_alloc();
    task_b->tss.esp = memman_alloc_4k(memman, 64*1024) + 64*1024 - 8; // SP to the end of a stack
    task_b->tss.eip = (int) &task_b_main; // entry point of task_b (4); IP = instruction pointer
    task_b->tss.es = 1*8;
    task_b->tss.cs = 2*8; // GDT 2 (this OS program)
    task_b->tss.ss = 1*8;
    task_b->tss.ds = 1*8;
    task_b->tss.fs = 1*8;
    task_b->tss.gs = 1*8;
