...
void HariMain(void) {
    ...
    char s[40];
    int fifobuf[128];
    int mx, my, i, args[10], cursor_x, cursor_c;
    ...
    for (i = 0; i < 3; i++) {
        ...
        args[0] = i;
        hrb_sprintfi(s, "task_b%d", args);
        make_window8(buf_win_b, 144, 52, s, 0);

        task_b[i] = task_alloc();
        task_b[i]->tss.esp = memman_alloc_4k(memman, 64*1024) + 64*1024 - 8; // SP to the end of a stack
        task_b[i]->tss.eip = (int) &task_b_main; // entry point of task_b (4); IP = instruction pointer
        task_b[i]->tss.es = 1*8;
        task_b[i]->tss.cs = 2*8; // GDT 2 (this OS program)
        task_b[i]->tss.ss = 1*8;
        task_b[i]->tss.ds = 1*8;
        task_b[i]->tss.fs = 1*8;
        task_b[i]->tss.gs = 1*8;
    ...
    for (;;) {
        ...
        if (fifo32_status(&fifo) == 0) {
            ...
            if (256 <= i && i <= 511) { // from a keyboard
                ...
                if (i < 256 + 0x54) {
                    if (keytable[i - 256] != 0 && cursor_x < 128) { // normal characters
                        s[0] = keytable[i - 256];
                        s[1] = 0;
